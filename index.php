...
// Creating Url
     $db_brand_url = parse_url($newbrand->url);
     $db_brand_url_path_count = strlen($db_brand_url['path']);

    echo $req_url['host'];
    $req_url = parse_url($_POST['req_item_url']);
//    $req_url = $req_url['scheme'] . "://" . $req_url['host'];
    $req_url_path = substr($req_url['path'],0, $db_brand_url_path_count);


    if ($req_url['host'] == $db_brand_url['host'] and  $db_brand_url['path'] == $req_url_path) {
        ...
        redirect('index.php?error=' . urlencode("Invalid Url"));
    }
}
if(isset($_POST['placeorder'])) {
    ...
    if ($customer->register($data)) {
        ...
        if ($request_order->register($data)) {
            ...
            OrderEmail($data, $item);
            session_unset();
            header("Location: thank-you.php?order=" . $data['request_id']);
            exit();
        }
    } else {
        echo 'Failed';
